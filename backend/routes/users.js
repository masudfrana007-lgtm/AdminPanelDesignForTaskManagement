...
router.post("/", auth, async (req, res) => {
  ...
  try {
    ...
    if (String(e).includes("users_email_key")) return res.status(409).json({ message: "Email already exists" });
...
router.get("/", auth, allowRoles("admin", "owner"), async (req, res) => {
  if (req.user.role === "admin") {
    const r = await pool.query("SELECT id, name, email, role, created_by, created_at FROM users ORDER BY id DESC");
